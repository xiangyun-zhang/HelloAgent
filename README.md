...
- **Web UI 基础框架**：基于 Gradio 5 构建的 Web 界面，支持对话交互与工具调用。
...
- [x] **M5: 长期记忆升级**（向量数据库 RAG）✅ 已完成
  ...
  - M6: 工程化收尾（Web UI 包装）⏳ 进行中
    - [x] **M6.1 核心解耦**：剥离 CLI 依赖，抽象 `on_status` 状态回调机制
    - [x] **M6.2 基础骨架**：接入 Gradio 5，打通 Web 端全链路对话与工具调用
    - [X] **M6.3 状态可视化**：在 Web 端拼接展示“思考链”与“沙箱执行日志”
    - [ ] **M6.4 打字机体验**：实现基于 SSE 的真正流式输出 (Streaming) - **已跳过**
    - [ ] **M6.5 界面增强 (可选)**：前后端历史记录同步、UI 快捷指令按钮等
